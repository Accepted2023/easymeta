...
const Plots = (function () {
  ...
  // Inverse normal CDF (for CI critical values)
  ...
  function forestPlot(container, studies, pooled, opts = {}) {
    ...
    let allLower = studies.map(s => s.TE - zCrit * s.seTE);
    let allUpper = studies.map(s => s.TE + zCrit * s.seTE);
    if (pooled) {
    ...
    const ciPct = (ciLevel * 100).toFixed(0);
    const effectLabel = expScale
      ? `${measure} [${ciPct}% CI]`
      : (measure === 'SMD' ? `SMD [${ciPct}% CI]` : measure === 'MD' ? `MD [${ciPct}% CI]` : `Effect [${ciPct}% CI]`);
    ...
    studies.forEach((study, i) => {
      ...
      // Effect size and CI
      const te = study.TE;
      const lower = te - zCrit * study.seTE;
      const upper = te + zCrit * study.seTE;
